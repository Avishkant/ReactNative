...
export default function ColorWheelClean({
  size = 280,
  onChange,
  hue = 0,
  sat = 100,
  ...
  const dots = useMemo(() => {
    ...
    for (let r = 0; r < rings; r++) {
      ...
      const segs = Math.max(
        6,
        Math.round(segments + r * Math.max(2, Math.round(rings / 3))),
      );
      const dotSize = Math.max(
        3,
        Math.round((size / (140 / dotScale)) * (1.0 + r * 0.12)),
      );
    ...
    >
      <View style={{ width: size, height: size }}>
        ...
        <View
          ...
          style={[
            styles.markerWrap,
            { left: marker.left - 14, top: marker.top - 14 },
          ]}
        >
          ...
          <View
            style={[
              styles.markerInner,
              { backgroundColor: hsvToHex(hue, sat) },
            ]}
          />
        </View>
      </View>
